...
//   - runs/{runId}.json      — one flow run's platform flowRunId / accumulated steps / outcome
//
...
// writeConfigMeta writes a scenario's demo-only meta sidecar (flow_id, connection_id, fixture) — run
// parameters that are NOT SDK Config fields, kept out of the canonical config file.
...
func (rt *Runtime) readConfigMeta(id int) map[string]any {
	m := readJSONMap(rt.metaPathFor(id))
	if m == nil {
